        ...
        public static function titleCase($title)
        {
            ...
            foreach ($m1[0] as &$m2) {
                ...
                ?	//convert first letter within wrapper to uppercase
					...
					substr ($m, 2, strlen ($m)-2)
                    
                //else:	do not uppercase these cases
				: (	preg_match ('/[\])}]/', substr ($title, max (0, $i-1), 3)) ||
					preg_match ('/[A-Z]+|&|\w+[._]\w+/', substr ($m, 1, strlen ($m)-1))
                ?	$m
                    //if all else fails, then no more fringe-cases; uppercase the word
				:	strtoupper (substr ($m, 0, 1)).
					substr ($m, 1, strlen ($m))
                ));
                
                //resplice the title with the change (`substr_replace` is not multi-byte aware)
				$title = substr ($title, 0, $i).$m.
					 substr ($title, $i+strlen($m), strlen ($title))
                ;
            }
